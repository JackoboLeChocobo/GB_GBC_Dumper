...
void cart_helper::print_cart_stuff()
{
	...
	switch (cart_mbc_type)
	{
		// No MBC at all
		case rom_only:
			Serial.println(F("MBC type:  None"));
			break;
		
		// MBC1
		case mbc1:
		case mbc1_ram:
			Serial.println(F("MBC type:  MBC1"));
			break;
		
		// MBC2
		case mbc2:
			Serial.println(F("MBC type:  MBC2"));
			break;
		...
		case mbc3_timer_ram:
		case mbc3_ram:
			Serial.println(F("MBC type:  MBC3"));
			break;
		
		// MBC5
		case mbc5:
		case mbc5_ram:
			Serial.println(F("MBC type:  MBC5"));
			break;
		
		// Something else
		case unknown_mbc_type:
			Serial.println(F("Unknown MBC type."));
			break;
	}
	
	Serial.print(F("This cartridge has "));
	Serial.print( rom_size, DEC );
	Serial.println(F(" ROM banks."));
	
	
	switch (ram_size)
	{
		case rs_none:
			Serial.println(F("This cartridge has no external RAM."));
			break;
		
		case rs_2:
			Serial.println(F("This cartridge has 2 kiB of external RAM."));
			break;
		
		case rs_8:
			Serial.println(F("This cartridge has 8 kiB of external RAM."));
			break;
		
		case rs_32:
			Serial.println(F("This cartridge has 32 kiB of external RAM."));
			break;
		
		case rs_128:
			Serial.println(F("This cartridge has 128 kiB of external RAM."));
			break;
		
		case rs_mbc2:
			Serial.println(F("This cartridge has no external RAM, but it does have an MBC2.  MBC2's have built-in RAM."));
			break;
...
uint8_t cart_helper::calc_tpak_bank( uint16_t gb_addr )
{
	...
	return (uint8_t)( gb_addr >> 14 );
}


uint16_t cart_helper::calc_tpak_addr( uint16_t gb_addr )
{
	return (uint16_t)( gb_addr | 0xc000 );
}



void cart_helper::read_with_gb_addr( uint16_t gb_addr )
{
...
int cart_helper::interpret_serial_message()
{
	...
	else if ( buf[0] == sm_gb_read_32_bytes )
	{
		...
		uint16_t gb_addr = gb_addr_pkt.w;
		
		//Serial.println( gb_addr, HEX );
		
		uint8_t tpak_bank = calc_tpak_bank(gb_addr);
		uint16_t tpak_addr = calc_tpak_addr(gb_addr);
		
		//uint8_t tpak_bank = 0x00;
		//uint16_t tpak_addr = 0xc000 + gb_addr;
	...
	else if ( buf[0] == sm_gb_read_var_num_bytes )
	{
		...
		// The GB address to read from is in buf[1] and buf[2] (uint16_t)
		gb_start_addr_pkt.hi = (byte)(buf[1]);
		gb_start_addr_pkt.lo = (byte)(buf[2]);
		
		
		// The number of bytes to read is in buf[3] and buf[4] (uint16_t)
		num_bytes_pkt.hi = (byte)(buf[3]);
		num_bytes_pkt.lo = (byte)(buf[4]);
		
		uint16_t gb_start_addr = gb_start_addr_pkt.w,
			num_bytes = num_bytes_pkt.w;
		
		uint8_t tpak_bank = calc_tpak_bank(gb_start_addr);
	...
	else if ( buf[0] == sm_gb_write_var_num_bytes )
	{
		...
		// The number of bytes to write is in buf[3] and buf[4] (uint16_t)
		num_bytes_pkt.hi = (byte)(buf[3]);
		num_bytes_pkt.lo = (byte)(buf[4]);
