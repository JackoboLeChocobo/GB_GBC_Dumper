...
void tpak::write( uint16_t write_addr )
{
...
void tpak::write( uint16_t write_addr, uint8_t* msg )
{
	addr_pkt.w = calc_addr_crc(write_addr);
...
void tpak::write_byte( uint16_t write_addr, uint8_t data )
{
	addr_pkt.w = calc_addr_crc(write_addr);
	//memcpy( &command[3], msg, 32 );
...
void tpak::read( uint16_t read_addr )
{
